...
from
(select bgye.gyar_id,min(if(bgye.io>=0,bgye.aktiv_db,if(bgye.aktiv_db*bgye.io+(be.db/1)*bgye.reszarany/1000000000>=0,bgye.aktiv_db,-(be.db/1)/1000000000*bgye.reszarany/bgye.io))) as effektiv_db
...
from (
	select bgye.bolygo_id,bgye.gyar_id,min(if(bgye.io>=0,bgye.aktiv_db,if(bgye.aktiv_db*bgye.io+(be.db/1)*bgye.reszarany/1000000000>=0,bgye.aktiv_db,-(be.db/1)/1000000000*bgye.reszarany/bgye.io))) as effektiv_db
	from bolygo_gyar_eroforras bgye,bolygo_eroforras be
...
?>,"term_mikor":<?
$term_mikor=($bolygo['id']%5)-((mysql2num('select idopont from ido')-1)%5);
if ($term_mikor<=0) $term_mikor+=1;
echo $term_mikor;
?>,"moral":<?
echo $bolygo['moral'];
?>,"lakohely":<?
