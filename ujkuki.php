...
function tokencsekk() {
	...
	if ($ismert) if ($adataim['token']==$_POST['token']) if (time()<strtotime($adataim['session_ervenyesseg'])) return true;
	return false;
}
...
/************************************************ HELYETTESÍTÉS ELEJE **************************************************************************/
/*if ($_POST['login_nev']!='') if (strpos($_POST['login_nev'],'#')!==false) {
	$ki_nev=substr($_POST['login_nev'],0,strpos($_POST['login_nev'],'#'));
	$kit_nev=substr($_POST['login_nev'],strpos($_POST['login_nev'],'#')+1);
	...
	$jelszo_hash=hash('whirlpool',$_POST['login_jelszo'].$ki_aux['jelszo_so'].$rendszer_so);
...
/************************************************ LOGIN ELEJE **************************************************************************/
/*if ($_POST['login_nev']!='') {
	$r=mysql_query('select * from userek where (nev="'.sanitstr($_POST['login_nev']).'" or email="'.sanitstr($_POST['login_nev']).'") and kitiltva=0 and inaktiv=0') or hiba(__FILE__,__LINE__,mysql_error());
	$aux=mysql_fetch_array($r);
	$jelszo_hash=hash('whirlpool',$_POST['login_jelszo'].$aux['jelszo_so'].$rendszer_so);
...
if (isset($_POST['del_reg'])) {
	if ((int)$uid>0) {
		$jelszo_hash=hash('whirlpool',$_POST['jelszo'].$adataim['jelszo_so'].$rendszer_so);
