...
,if(b.tulaj_szov!='.$adataim['tulaj_szov'].' and (coalesce(dsz.mi,0)=1 or dsz.mi is null),1,0)
,b.tulaj_szov
,if(b.tulaj='.$uid.',1,if(b.tulaj_szov='.$adataim['tulaj_szov'].',2,if(coalesce(dsz.mi,0)=2,3,0)))
from hexak h
inner join bolygok b on h.voronoi_bolygo_id=b.id
left join userek u on u.id=b.tulaj
left join diplomacia_statuszok dsz on dsz.ki='.$adataim['tulaj_szov'].' and dsz.kivel=b.tulaj_szov
where coalesce(dsz.mi,0)!=3
