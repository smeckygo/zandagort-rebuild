...
$kivalasztott_szovi=mysql2row('select * from szovetsegek where id='.((int)$_POST['sz']));
if ($kivalasztott_szovi) {
} else {
	$kivalasztott_szovi['id']=0;
}
if (isset($_POST['u'])) {
	$kivalasztott_user=mysql2row('select * from userek where id='.((int)$_POST['u']));
...
<h2><?
if ($_GET['k']>1) echo '<a href=".?k='.($_GET['k']-1).'">&larr;</a> ';
?><?=$lang[$lang_lang]['top/index.php']['. héten regisztrált játékosok prefix'];?><?=$_GET['k']?><?=$lang[$lang_lang]['top/index.php']['. héten regisztrált játékosok'];?><?
$max_kohort=mysql2num('select max(floor(timestampdiff(day,"'.$szerver_indulasa.'",mikortol)/7)+1) from userek');
if ($_GET['k']<$max_kohort) echo ' <a href=".?k='.($_GET['k']+1).'">&rarr;</a>';
?></h2>
<table>
<?
...
where u.szovetseg not in ('.implode(',',$specko_szovetsegek_listaja).') and u.id not in ('.implode(',',$specko_userek_listaja).')
and floor(timestampdiff(day,"'.$szerver_indulasa.'",u.mikortol)/7)+1='.$_GET['k'].'
and (u.karrier!=3 or u.speci!=3)
group by u.id
...
?>" /></td>
	<td><a href=".?u=<?=$aux['id'];?>"><?=$aux['nev'];?></a></td>
	<td><?=number_format($aux[1],0,$lang[$lang_lang]['top/index.php'][','],' ');?></td>
<td style="text-align: right"><?=number_format($aux['db'],0,$lang[$lang_lang]['top/index.php'][','],' ');?></td>
<td><?=$aux['szovrovnev'];?></td><td><?=$aux['szovnev'];?></td>
</tr>
<? } ?>
